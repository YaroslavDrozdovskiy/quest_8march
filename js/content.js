/* ═══════════════════════════════════════════════
   КОНТЕНТ — весь текст проекта здесь
   Меняй этот файл, чтобы редактировать тексты
   ═══════════════════════════════════════════════ */

// Секретный код замка
const SECRET_CODE = '1804';

// Призы: { name, poem, img? }
// Чтобы добавить приз — скопируй объект ниже и заполни поля:
// { name: 'Название', poem: 'Текст стихотворения', img: 'assets/имя_файла.jpg' }
// Поле img необязательно — без него покажется иконка-заглушка 🎁
const CERTIFICATES = [
  {
    name: 'Подарочный сертификат',
    poem: 'Не конверт, не бант, не пакет,<br>Электронный сюрприз — вот секрет!<br>В 19:00, без хлопот,<br>«Золотое Яблоко» в дом придёт!',
    img: 'assets/sertificate.jpg',
  },
];

const CONTENT = {

  // Вкладка браузера
  pageTitle: 'Сказ о Сундуке Заветном',

  // ─── Сцена 1: Вступление ────────────────────
  intro: {
    eyebrow:  '✦ 8 Марта ✦',
    title:    'Сказ о Сундуке<br>Заветном',
    tagline:  'Сокровище ждёт своих хозяек',
    verse:    'Слух прошёл в краю честном —<br>' +
              'Будто клад в ларце чудесном<br>' +
              'Ждёт красавиц дорогих.<br>' +
              'Краше нет на свете их!',
    startBtn: 'В путь-дорогу!',
  },

  // ─── Сцена 2: Замок ─────────────────────────
  chest: {
    title:      'Замок на сундуке крепок!',
    hint:       'Вращайте барабаны — назовите<br>' +
                'Четыре заветных цифры, что знаете',
    openBtn:    'Отворить!',
    errorWrong: 'Замок не поддался! Попробуйте ещё раз...',
    arrowUp:    'вверх',
    arrowDown:  'вниз',
  },

  // ─── Сцена 3: Открытие ──────────────────────
  opening: {
    verse: 'Скрипнул ларчик золотой...<br>Свет волшебный — пред тобой!',
  },

  // ─── Сцена «Свиток» ─────────────────────────
  scroll: {
    hint:    '↓ листайте',
    nextBtn: 'Далее ✦',
    title:   '⚜ Свиток из казны Сберградовской',
    body:
`Сударыни мудрые и отважные!

Вы разгадали тайну чисел
И открыли казну, что скрыта была
От глаз недостойных.

Но знайте:
рыцари Ливонского ордена
боятся не меча, не копья
и даже не богатырской силы.

Больше всего на свете
страшатся они ведьминских проклятий.

Посему, когда явятся они за казной,
не медлите и не страшитесь.

Говорите громко, единогласно,
и осыпьте их древними заклятиями —
и обратятся они в бегство.

Вот слова силы:

— Да чтоб латы ваши ржавчиной покрылись!
— Чтоб мечи ваши в лапшу согнулись!
— Чтоб шлемы ваши на глаза съехали!
— Чтоб кони ваши задом наперёд ходили!
— Чтоб сапоги ваши к полу прилипли!
— Чтоб щиты ваши мыши прогрызли!
— Чтоб бороды ваши узлом завязались!
— Чтоб доспехи ваши скрипели громче телеги!
— Чтоб шпоры ваши в узел свернулись!
— Чтоб меч ваш от ножен убегал!

Кричите дружно, не жалея голоса —
и не устоит перед вами
ни один рыцарь заморский.

А когда прогоните их прочь,
освободите своих богатырей
и празднуйте победу.

Ибо казна сия —
лишь малая награда
за храбрость вашу и мудрость.

Слава вам,
сударыни-победительницы!`,
  },

  // ─── Сцена 4: Награда ───────────────────────
  reward: {
    title:     'С 8 Марта, красавицы!',
    body:
      'Вы — сокровища из сказки,\n' +
      'Драгоценней всех на свете!\n' +
      'Принимайте дары с лаской —\n' +
      'Знайте: любим вас, поверьте!\n\n' +
      'Пусть сундук сей будет знаком,\n' +
      'Что вы — прекрасны, нет предела!\n' +
      'Идём к вам радостным мы шагом —\n' +
      'Дарить вам радость — наше дело!',
    cardLabel: 'дар заветный',
  },

};
